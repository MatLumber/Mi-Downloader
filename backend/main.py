...
class DownloadRequest(BaseModel):
    ...
    cookies_file: Optional[str] = None  # absolute path to Netscape-format cookies.txt (overrides browser)
...
class SyncCookieEntry(BaseModel):
    domain: str
    name: str
    value: str
    path: Optional[str] = "/"
...
class SyncCookiesStatus(BaseModel):
    exists: bool
    path: Optional[str]
...
# --- Companion extension cookie sync ---
#
# The browser extension at frontend/companion-extension/ POSTs YouTube/Google cookies
# here. We persist them in Netscape format at a stable path under %LOCALAPPDATA%, and
# the downloader auto-uses that file when the user has cookies enabled and hasn't
# selected a manual cookies.txt. This bypasses Chrome 127+'s app-bound encryption
# bug entirely because chrome.cookies (an extension API) returns plaintext values.

_COOKIES_LOCK = threading.Lock()
_COOKIES_LAST_SYNC: dict = {"timestamp": None, "count": 0}


def _synced_cookies_path() -> str:
    base = os.environ.get("LOCALAPPDATA") or os.path.expanduser("~/AppData/Local")
    folder = os.path.join(base, "GravityDown")
    os.makedirs(folder, exist_ok=True)
    return os.path.join(folder, "synced-cookies.txt")


def _to_netscape(cookies: list) -> str:
    """Convert a list of cookie dicts to Netscape cookies.txt format consumable by yt-dlp."""
    lines = [
        "# Netscape HTTP Cookie File",
        "# Generated by GravityDown Companion extension",
    ...
    for c in cookies:
        ...
        if not domain or not name:
            continue
        # In Netscape format the include-subdomains flag is TRUE iff the domain
        # starts with a leading dot. chrome.cookies preserves that convention.
        ...
        try:
            ...
            expires = 0
        # Tabs and newlines are field separators in this format — strip defensively.
...
@app.post("/cookies/sync")
async def cookies_sync(request: SyncCookiesRequest):
    if not request.cookies:
        raise HTTPException(status_code=400, detail="No cookies provided")
    path = _synced_cookies_path()
    content = _to_netscape(request.cookies)
    try:
        with _COOKIES_LOCK:
            with open(path, "w", encoding="utf-8", newline="\n") as f:
                f.write(content)
            _COOKIES_LAST_SYNC["timestamp"] = time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime())
            _COOKIES_LAST_SYNC["count"] = len(request.cookies)
    ...
    return {
        ...
        "timestamp": _COOKIES_LAST_SYNC["timestamp"],
    }


@app.get("/cookies/sync-status", response_model=SyncCookiesStatus)
async def cookies_sync_status():
    path = _synced_cookies_path()
    exists = os.path.isfile(path)
    return SyncCookiesStatus(
        exists=exists,
        path=path if exists else None,
        timestamp=_COOKIES_LAST_SYNC["timestamp"],
        count=_COOKIES_LAST_SYNC["count"],
    )


@app.delete("/cookies/sync")
async def cookies_sync_clear():
    path = _synced_cookies_path()
    try:
        if os.path.isfile(path):
            os.remove(path)
        with _COOKIES_LOCK:
            _COOKIES_LAST_SYNC["timestamp"] = None
            _COOKIES_LAST_SYNC["count"] = 0
        return {"ok": True}
    except OSError as exc:
